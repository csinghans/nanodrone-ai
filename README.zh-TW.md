...
🚀 **完全新手？**先讀[**從這裡開始**](docs/zh-TW/START-HERE.md)——一句話目標、你的第一次飛行、以及課程地圖。卡在某個詞？[術語表](docs/zh-TW/GLOSSARY.md)用白話解釋課程的關鍵詞。
...
| **6** | 跟隨模式（視覺伺服） | $0 | 用相機跟隨移動目標的無人機 |
| **7** | 跟著飛手 | $0 | 你開「人」到處走，無人機自動轉向尾隨 |
| **8** | 跟隨真人（CNN） | $0 | 用*訓練出*的偵測器跟隨色彩規則找不到的人 |
| **9** | 語音操控飛行 | $0 | 用說的飛 —— 離線語音模型，支援中英文 |
| **10** | 訓練自己的語音命令 | $0 | 用*你的*聲音訓練關鍵詞辨識模型 |

**Lesson 10 之後**（後續 roadmap，已實作 —— 見 [docs/zh-TW/ROADMAP.md](docs/zh-TW/ROADMAP.md)）：

| 課程 | 主題 | 成本 | 你會做出 |
|------|------|------|----------|
| **11** | 任務狀態機 + failsafe | $0 | 把多階段（起飛→飛→降落）編排在一台 `nanodrone.mission` 狀態機上 |
| **12** | 語音驅動的階段轉移 | $0 | 口語命令切換整個階段，而非只是持續微調 |
| **13** | 多模態 mini-capstone | $0 | 起飛→找到人→跟隨→喊降落就降落（再訓一個確認 CNN） |
| **14a** | 建圖與自主巡邏 | $0 | 邊巡邏邊用深度建出 2D 佔據地圖 |
| **14b** | 板載預算（量化+延遲） | $0 | 把感知縫回 GAP8：int8 塞得下 + 延遲對追蹤曲線 |
| **16** | 畢業專題 | $0 | 設計並評分*你自己*的任務（範本 + 驗證器 + rubric） |
| **17** | 單目深度估計 | $0 | 從單一相機訓一個 dense 深度 CNN（第一個 dense 模型） |
| **18** | 光流 / 視覺里程計 | $0 | 用朝下相機、無 GPS 自估運動 |
| **19** | 更難的多障礙 RL | $0 | 在隨機佈局上泛化避障（把感知併入觀測） |
| **20** | 域隨機化 | $0 | 擾動訓練影像以縮小 sim-to-real 落差 |
| **21** | 蒸餾 + 壓縮 | $0 | 把模型縮 ~6 倍且保留準確度 |
| **22** | 統一板載策略 | $0 | 把避障 + 跟隨蒸餾成單一小網路 |
| **23** | 飛行黑盒子 | $0 | 遙測記錄 + 忠實回放 |
| **24** | 第一台真機（Tello） | 約 US$100 | 同協定飛 Wi-Fi Tello，加一層可重用 safety |
| **25** | 量測 sim-to-real 落差 | $0 | 量化 sim 模型在仿真實相機上掉多少 |
| **26** | 實地測試 SOP + 法規 | $0 | GO/NO-GO 飛行前 gate + 台灣民航局查核清單 |
| **27** | 抽出飛行協定 | $0 | 一套 schema（`nanodrone.protocol`）到處重用 |
| **28** | 解析器擂台（反例） | $0 | 何時通用模型 + 約束勝過自訓小模型 |
| **29** | nano 世界模型（V-JEPA，板載） | $0 | 預測四個 horizon 的*下一個隱空間*；在它之上學出的策略把 0.8–1.6 m/s 全速度帶零墜機飛完、反應式尾端 60%（板載預算 137KB） |
| **30** | 課程總結 | $0 | 用數字回望整條弧線，外加一個重新驗證共用合約的畢業檢查 |
