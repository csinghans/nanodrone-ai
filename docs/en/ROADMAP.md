...
**Going further**: (1) **geometry-grounded latent prediction** — use 4D-GS *offline* (sim / Orin) to produce metric occupancy and add a grounding loss so `ẑ_{t+k}` decodes to a collision-checkable distance, buying V-JEPA's latency with 4D-GS's grounding (an ICRA/CVPR-flavoured contribution); (2) swap `proactive_avoid`'s privileged look-ahead for the trained collision head (vision-in-the-loop, the honest L13 fallback convention); (3) feed the predicted time-to-collision into L19's RL observation for a *learned* proactive avoider. Honest gap: full V-JEPA 2 / 4D-GS are Orin-class — this teaches the principle under the GAP8 budget.
