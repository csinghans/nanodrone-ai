...
> Track B ordering principle: first unlock new perception modalities (depth, optical flow) → feed them into harder decisions (multi-obstacle RL) → close the sim-to-real gap (domain randomization) → slim down for on-board (distillation / compression) → unify capabilities (distillation capstone).
...
Lesson order: L11 → L12 → L13 → L14a → L14b → L16 → L17 → L18 → L19 → L20 → L21 → L22 → L23 → L24 → L25 → L26 → L27 → L28 → DroneVoice Phase 2 → Phase 3 → Phase 4 → Phase 5a → Phase 5b.
